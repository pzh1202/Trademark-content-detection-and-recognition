...
def detect_bar(img_bar):
    ...
    for barcode in barcodes:
        ...
        cv2.rectangle(img_bar, (x, y), (x + w, y + h), (255, 255, 0), 2)
        ...
        frame = cv2.putText(img_bar, barcodeData, (x, y - 25), cv2.FONT_HERSHEY_PLAIN,
                            1, (0, 255, 0), 2)
        return img_bar,barcodeData

def det_health_tag(img_path, img, tag_path = 'test2.png'):
    MIN_MATCH_COUNT = 10
...
def reg_on_wind(img_path1, img_save_path, dict_result):
    dict_result_content = {'商标': 'false', '经销企业': 'false', '不适宜人群': 'false', '许可证编号': 'false', '执行标准': 'false', '条形码': 'false',
                   '保健品商标': 'false'}

    flag = 0
    img = cv2.imread(img_path1)
    if dict_result['商标'] == True:
        flag = 1
        print('商标')
    ...
    if dict_result['条形码'] ==True:
        img, barcodeData = detect_bar(img)
        flag = 1
        print(barcodeData)
    if dict_result['保健品商标'] == True:
        img, flag_B = det_health_tag(img_path1, img)
        print(flag_B)
        flag = 1
    if flag == 0:
        img = cv2.imread(img_path1)
    img_res = img_resize(img)
    cv2.imwrite('12.png', img_res)

def recog_label(img_path1, img_save_path, dict_result):
    orientations = 9
    pixels_per_cell = (2, 2)
    cells_per_block = (2, 2)
    threshold = .3
    ...
    dict_result_result = {'商标':'false', '经销企业':'false', '不适宜人群':'false', '许可证编号':'false', '执行标准':'false','条形码': 'false',
                   '保健品商标': 'false'}
    dict_result_content = {'商标':'false', '经销企业':'false', '不适宜人群':'false', '许可证编号':'false', '执行标准':'false','条形码': 'false',
                   '保健品商标': 'false'}
